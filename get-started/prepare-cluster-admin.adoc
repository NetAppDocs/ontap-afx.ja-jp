...
NOTE: ONTAP 9.19.1以降、SVM管理者は専用のSystem Managerダッシュボードを使用して、自身のSVMを監視および管理できます。詳細については、 https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager SVM管理者ダッシュボード"^]
...
AFXでは、クラスタの設定および使用時に、最小値や最大値など、いくつかの制限が適用されます。これらの制限はいくつかのカテゴリに分類されます。


CAUTION: AFXストレージシステムの運用パラメータを上げる前に、それがアプリケーションおよびクラスタ全体にどのような影響を与えるかを評価する必要があります。例えば、コントローラノードの数を増やすと全体の容量は増加するものの、クラスタネットワーク内のトラフィックも増加する可能性があります。これらの設定変更がもたらす影響と、それがお客様の環境にどのような影響を与える可能性があるかを理解することが重要です。

クラスターあたりのコントローラーノード:: 各AFXクラスターは、少なくとも4つのノードで構成されている必要があります。ノードの最大数はONTAPリリースによって異なります。たとえば、9.19.1リリースでは、AFXクラスターあたり最大32ノードまで使用可能です。
ストレージ容量:: これは、クラスターのストレージ可用性ゾーン (SAZ) 内のすべての SSD ディスクの合計容量です。最大ストレージ容量は、 ONTAPリリースによって異なります。
クラスタースイッチ:: クラスター ストレージ ネットワークには少なくとも 2 つのスイッチが必要です。許容される最大値は、クラスター内のコントローラー ノードの合計数に基づいて決定されます。


NetApp Hardware Universeおよび相互運用性マトリックスツールで入手可能な詳細を確認して、AFXクラスタの機能を判断する必要があります。また、AFXリリースノートセクションのlink:../release-notes/changes-to-limits-defaults.html["ONTAPの制限とデフォルトの変更点"]も確認してください。
