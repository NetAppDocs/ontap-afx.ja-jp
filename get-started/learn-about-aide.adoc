...
[role="lead"]
NetApp AI Data Engine（AIDE）は、AFXクラスタに接続されたコンピューティングノード上で動作する、ストレージ統合型AIデータサービスです。オプションで、次の2つの導入モデルのいずれかを使用して、AIDEをAFXクラスタにインストールして統合できます：NetApp提供のデータコンピューティングノード（DCN）、またはサードパーティサーバー上のAIDEソフトウェア。NetApp DCN導入では完全なAIDE機能が提供されますが、サードパーティサーバー導入はメタデータに重点を置いています。
